...
#Escolha da loja
loja= input("Digite a unidade da loja: ")


import json 

with open ('estoque{0}.json'.format(loja),'r') as entrada:
...
#opção de menu 1
    if escolha=="1":
        ...
        if produto !="0":
            ...
            with open ('estoque{0}.json'.format(loja),'w') as saida:
...
#opção de menu 2
        ...
        estoque_json = json.dumps(estoque, sort_keys=True, indent=4)
        with open ('estoque.json{0}'.format(loja),'w') as saida:
            saida.write(estoque_json)
